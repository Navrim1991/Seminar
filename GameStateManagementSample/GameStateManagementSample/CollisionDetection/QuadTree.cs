    ...
    public class QuadTree<T> where T : class, IQuadObject
    {
        ...
        public void Insert(T quadObject)
        {
            lock (syncLock)
            {
                ...
                if (root == null)
                {
                    //var rootSize = new Point(Math.Ceiling(bounds.Width / minLeafSize.X), Math.Ceiling(bounds.Height / minLeafSize.Y));
                    var rootSize = new Point((int)Math.Ceiling((double)bounds.Width / (double)minLeafSize.X),
                        (int)Math.Ceiling((double)bounds.Height / (double)minLeafSize.Y));
                    double multiplier = Math.Max(rootSize.X, rootSize.Y);
                    //rootSize = new Vector2(minLeafSize.X * multiplier, minLeafSize.Y * multiplier);
                    rootSize = new Point((int)(minLeafSize.X * multiplier),(int)(minLeafSize.Y * multiplier));
                    var center = new Point(bounds.X + bounds.Width / 2, bounds.Y + bounds.Height / 2);
                    var rootOrigin = new Point(center.X - rootSize.X / 2, center.Y - rootSize.Y / 2);
                    root = new QuadNode(new Rectangle(rootOrigin, rootSize));
                }
            ...
            lock (syncLock)
            {
                ...
                if (bounds.Intersects(node.Bounds))
                {
                    foreach (T quadObject in node.Objects)
                        if (bounds != quadObject.Bounds && bounds.Intersects(quadObject.Bounds))
        ...
        private void ExpandRoot(Rectangle newChildBounds)
        {
            lock (syncLock)
            {
                ...
                int newX = (rootDirection == Direction.NW || rootDirection == Direction.SW)
                                  ? root.Bounds.X
                                  : root.Bounds.X - root.Bounds.Width;
                int newY = (rootDirection == Direction.NW || rootDirection == Direction.NE)
                                  ? root.Bounds.Y
        ...
        void quadObject_BoundsChanged(object sender, EventArgs e)
        {
            ...
            {
                if (!(sender is T))
                    throw new ArgumentException("Sender Entspricht nicht dem Generische Typen");
